...
A live dogfood run of the risk-review prompt against install.py's current code found and
...
root was actually deleted (docs/dogfooding.md, 2026-08-14).
...
fallback. tests/test_install.py grows by 3. The scoped-out broader symlink question (a fixed
kit-owned path itself behind a pre-planted symlink) stays open, named here for whoever picks it up
next, not resolved by this fix.
