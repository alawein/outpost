...
`python tools/run_evals.py` runs 9 pilot prompts through a real `claude -p` call against a seeded
...
`write-tests`, `debt-log`, `orient-repo`, `triage`, `repo-review`, `code-review`);
`python tools/run_evals.py --only <name>` runs just one.
