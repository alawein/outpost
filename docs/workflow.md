...
| Review and ship | `check-intent` `code-review` `risk-review` `prepare-pr` `respond-to-review` `triage` `handoff-session` | review, draft the PR, answer feedback, hand off |
| Converge | `converge` | drive an artifact to clean over lint, tests, review, and scrutiny rounds |
| Scrutiny | `self-refute` `grill` `prove` `panel` `repo-review` `cross-doc-check` `repo-hygiene-sweep` | challenge your own fresh output, a design, plan, or diff, or claims that must survive scrutiny; convene expert lenses for a wide decision |
