...
4. Regenerate the plugin so its manifest version matches: `python tools/build.py plugin`. The `plugin_sync` check fails if the committed plugin tree no longer matches the catalog. On Windows the generator may rewrite line endings. `.gitattributes` normalizes to LF on staging, so stage only the real change (the `plugin.json` version line), not unchanged `SKILL.md` files.
