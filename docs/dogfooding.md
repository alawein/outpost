...
| 2026-08-07 | check-intent | Ran the prompt's own steps against this admitting change's diff, plan: independent prompt contract, independence verification, catalog entry, regenerated docs and plugin spans, admission ADR, dogfood row, contract tests, changelog entry. First pass: every planned item traced Done, but `pytest` then failed three tests (`test_load_prompts_none_returns_all`, `test_converge_ships_to_claude_only`, `test_docs_sync_catches_a_hand_edited_generated_span`) with hardcoded 25/24-prompt counts the plan never named, because the plan listed "contract tests" as one item and did not enumerate the count-bump fallout in the existing suite. Fixed and re-ran; second pass clean. | Keep. The gap it found is in its own admitting run: a plan item stated at the wrong grain ("contract tests" instead of naming the count-bump fixups) reads as Done while real fallout ships untested. Worth narrowing the prompt's guidance on plan-item grain in a future revision if this recurs. |
