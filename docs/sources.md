...
`--source` repeats for more than one library. It works with `--dry-run`, `--verify`, `--prune`, and `--remove` the same way the core prompts do, and `--only` and `--exclude` apply to source skill names too. Without `--source`, only the core prompts install.

The manifest (`.outpost/manifest.json`) records each source's path once, and per tool the skills that tool kept from it, since installs are per tool: a source installed for Codex alone is never demanded of Claude. The installed files land under each tool's `files` map like any other kit-written path. A later `--verify` or `--prune` with no `--source` reads the recorded path back and checks the installed copies against the source's current state. A recorded path that no longer exists prints `SOURCE MISSING <name> <path>` and exits 1.
...
| windsurf | `.windsurf/workflows/<source>-<name>.md` | `SKILL.md` verbatim; a skill over 12,000 characters is skipped, never truncated |
...
- One name per skill. A source skill named like a core prompt or like another source's skill stops the install with an error naming the path; nothing is written. Two clones with the same directory name are rejected the same way, so rename one clone.
...
`--verify` compares every installed copy, byte for byte, against the source's current state. Pull the clone and every installed copy of a skill that changed reads `DRIFTED` until you re-install. That is the point, and this is how to see it:
